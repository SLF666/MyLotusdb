...
// 批处理是数据库的批量操作。
// 如果只读属性为 true，您只能通过 Get 方法从批处理中获取数据。
// 如果尝试使用 Put 或 Delete 方法，将返回错误。
//
// 如果只读属性为 false，您可以使用 Put 和 Delete 方法向批处理中写入数据。
// 当您调用 Commit 方法时，数据将被写入数据库。
//
// 批处理不是事务，不保证隔离性。
// 但它可以保证原子性、一致性和持久性（如果 Sync 选项为 true）。
//
// 您必须调用 Commit 方法来提交批处理，否则数据库将被锁定。
type Batch struct {
	db            *DB
