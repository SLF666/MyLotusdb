...
type Options struct {
	...
	// MemtableSize 表示 memtable 的最大大小（以字节为单位）。
	// 这意味着每个 memtable 将占用如此多的内存。
	// 默认值为 64MB。
	MemtableSize uint32

	// MemtableNums 表示在刷新之前内存中保留的 memtable 的最大数量。
	// 默认值为 15。
	MemtableNums int

	//是否启用同步写入。默认是 false，如果设置为 true，则会使用 fsync 来确保数据持久化。
	Sync bool
